...
import {
  LayoutDashboard, ArrowLeftRight, Scale, FileBarChart, Settings,
  HandCoins, Sparkles, Heart, PiggyBank, Receipt, Landmark, MoreHorizontal,
  Wallet, ChevronDown,
} from 'lucide-react';
...
export function useGroupOpen(group: NavGroup): [boolean, (v: boolean) => void] {
  ...
  const persistedSet = (v: boolean) => {
    setOpen(v);
    try { window.localStorage.setItem(STORAGE_PREFIX + group.id, v ? 'open' : 'closed'); } catch { /* ignore */ }
  };
...
export function leafLinkClass(isActive: boolean, options: { collapsed?: boolean; nested?: boolean } = {}) {
  ...
  return cn(
    ...
    collapsed ? 'justify-center px-2 py-2.5' : nested ? 'px-3 py-2 pl-9 text-[13px]' : 'px-3 py-2.5',
    isActive
...
interface ActiveBarProps { show: boolean }
export function ActiveBar({ show }: ActiveBarProps) {
  if (!show) return null;
  return <span className="absolute left-0 top-1/2 h-6 w-[3px] -translate-y-1/2 rounded-r-full bg-violet-600 dark:bg-violet-400" />;
}
...
export function GroupHeader({ group, open, onToggle, collapsed, containsActive }: GroupHeaderProps) {
  return (
    <button
      type="button"
      onClick={onToggle}
    ...
    >
      ...
      <group.icon className={cn('h-[18px] w-[18px] shrink-0', containsActive && 'text-violet-600 dark:text-violet-400')} />
      {!collapsed && (
        <>
          <span className="flex-1 text-left truncate">{group.label}</span>
          <ChevronDown className={cn('h-3.5 w-3.5 shrink-0 transition-transform duration-200', open ? 'rotate-0' : '-rotate-90')} />
        </>
      )}
    </button>
  );
...
export function LeafLink({
  leaf, collapsed, nested, onNavigate,
}: { leaf: NavLeaf; collapsed?: boolean; nested?: boolean; onNavigate?: () => void }) {
  return (
    <NavLink
      to={leaf.to}
      end={leaf.to === '/'}
      onClick={onNavigate}
      className={({ isActive }) => leafLinkClass(isActive, { collapsed, nested })}
    >
      {({ isActive }) => (
        <>
          {!nested && <ActiveBar show={isActive} />}
          <leaf.icon className={cn('h-[18px] w-[18px] shrink-0', isActive && 'text-violet-600 dark:text-violet-400')} />
